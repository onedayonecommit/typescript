...
// 제네시스 블록 설정값 적용
// geth --datadir /Users/gyeonghwan/go-ethereum/node init /Users/gyeonghwan/Download/20221122/genesis.json
// 실행 명령어
// geth --datadir node
...
// 특정 계정 unlock 시켜주기
// geth --datadir node --http --http/addr "127.0.0.1" --http.port 9000 -- http.corsdomain "*" \ -http.api "admin,eth,debug,miner,net,txpool,prosonal,web3" --syncmode full --networkid 0609\ --port 30300 --ws --ws.addr "127.0.0.1" --ws.port 9005 --ws.origins "*" \ --ws.api "admin,eth,debug,miner,net,txpool,personal,web3" \ --allow-insecure-unlock --unlock "0,1" --password "./node/password.txt"

// 자바스크립트 콘솔창에
// bytecode = "0x" 뒤에 솔리디티 bin파일 내용 붙여넣기

// abi = 솔리디티로 컴파일한 abi 파일의 내용을 붙여넣어줌
// 자바스크립트 콘솔창에서 abi 변수에 값을할당

// 트랜잭션 객체 만듬
// from 키값과 data 키값으로 객체 생성
// txObject = { from : eth.coinbase, data : bytecode}
// eth.sendTransaction(위에서 만든 트잭 객체 txObject)

// eth.getTransaction(트랜잭션의 해시값)

// contract 변수에 eth.contract(위의 abi 값)을 할당하여
// 만든 객체가 컨트랙트 객체이다.
// contract.at() at함수를 사용해서 스마트 컨트랙트 코드에 접근 가능.
